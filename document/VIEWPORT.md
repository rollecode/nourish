...
### I5. Default/primary-monitor selection retired from the Display tab
- The Display tab's "select a different monitor + Apply" used to route through the
  active-output **switch** gate (`OutputSwitchRequest`), which persisted a preferred
  default monitor (`pref::set_default`). That flow is retired: a resolution change to
  any lit monitor is now an in-place per-pipe mode change (`switch: false`), because
  every connected output is independently driven. Consequence: there is currently **no
  UI to set the default/primary monitor**. If that's still wanted in multi-output, it
  needs a dedicated affordance + a rethink of what "primary" means (layout origin?
  `profiles.first()` at startup?) — the `OutputSwitchRequest` machinery still exists
  (`display.switch`) but is no longer triggered from the resolution UI.
...
- Outputs are mapped into the `Space` by deterministic horizontal width-tiling
  ...
  `display.switch::add_output`). Because rendering is per-output-camera and
