...
impl<'a> Widget<SettingsMessage, Theme, Renderer> for LayoutCanvas<'a> {
    ...
    fn draw(
        ...
        renderer.with_layer(bounds, |renderer| {
            ...
            let order = self
                .placements
                .iter()
                .filter(|p| self.selected != Some(p.id))
                .chain(self.placements.iter().filter(|p| self.selected == Some(p.id)));
            for p in order {
    ...
    fn update(
        ...
        match event {
            ...
            Event::Mouse(mouse::Event::ButtonPressed(mouse::Button::Left)) => {
                ...
                // Otherwise select + begin moving the topmost square under the cursor.
                if let Some(p) = self.placements.iter().rev().find(|p| point_in(lx, ly, p)) {
    ...
    fn mouse_interaction(
        ...
        if let Some(pos) = cursor.position_in(layout.bounds()) {
            ...
            if self.placements.iter().any(|p| point_in(lx, ly, p)) {
