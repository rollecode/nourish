...
//! No logging dep (sits below logging so [`init`] runs first) — unrecognized flags
//! go to [`unknown`] for a logging-capable site to warn.
...
bitflags! {
    ...
    pub struct GpuFlags: u16 {
        ...
        const PIN_WGPU_NODE = 1 << 7;
    }
}

fn bit_for(flag: &str) -> Option<GpuFlags> {
    Some(match flag {
        "gpu_negotiate_modifiers" => GpuFlags::NEGOTIATE_MODIFIERS,
        ...
        "gpu_pin_wgpu_node" => GpuFlags::PIN_WGPU_NODE,
        _ => return None,
    })
}
